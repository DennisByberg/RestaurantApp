...
namespace RestaurantApp.Data
{
    public class ApplicationDbContext : IdentityDbContext<ApplicationUser>
    {
        ...
        {
            // Call the base class's OnModelCreating method to ensure any configurations
            // defined in the base class are applied.
            base.OnModelCreating(modelBuilder);

            // Configure the ProductIngredient entity to use a composite primary key
            // consisting of both ProductId and IngredientId.
            modelBuilder.Entity<ProductIngredient>()
                .HasKey(pi => new { pi.ProductId, pi.IngredientId });

            // Configure the relationship where each ProductIngredient has one Product
            // Configure the relationship where a Product can have many ProductIngredients
            // Specify that the foreign key in the ProductIngredient entity is ProductId
            modelBuilder.Entity<ProductIngredient>()
            .HasOne(pi => pi.Product)
            .WithMany(p => p.ProductIngredients)
            .HasForeignKey(pi => pi.ProductId);

            // Configure the relationship where each ProductIngredient has one Ingredient
            // Configure the relationship where an Ingredient can have many ProductIngredients
            // Specify that the foreign key in the ProductIngredient entity is IngredientId
            modelBuilder.Entity<ProductIngredient>()
